...
MKL_DallasTemperature::MKL_DallasTemperature() {
#if REQUIRESALARMS
...
MKL_DallasTemperature::MKL_DallasTemperature(MKL_OneWire* _oneWire) : MKL_DallasTemperature() {
	setOneWire(_oneWire);
}

bool MKL_DallasTemperature::validFamily(const uint8_t* deviceAddress) {
 ...
 */
MKL_DallasTemperature::MKL_DallasTemperature(MKL_OneWire* _oneWire, uint8_t _pullupPin) : MKL_DallasTemperature(_oneWire) {
  setPullupPin(_pullupPin);
}

void MKL_DallasTemperature::setPullupPin(uint8_t _pullupPin) {
...
void MKL_DallasTemperature::setOneWire(MKL_OneWire* _oneWire) {
...
// initialise the bus
void MKL_DallasTemperature::begin(void) {
...
uint8_t MKL_DallasTemperature::getDeviceCount(void) {
	return devices;
}

uint8_t MKL_DallasTemperature::getDS18Count(void) {
	return ds18Count;
}

// returns true if address is valid
bool MKL_DallasTemperature::validAddress(const uint8_t* deviceAddress) {
	return (_wire->crc8(deviceAddress, 7) == deviceAddress[DSROM_CRC]);
}

// finds an address at a given index on the bus
// returns true if the device was found
bool MKL_DallasTemperature::getAddress(uint8_t* deviceAddress, uint8_t index) {
...
bool MKL_DallasTemperature::isConnected(const uint8_t* deviceAddress) {
...
bool MKL_DallasTemperature::readScratchPad(const uint8_t* deviceAddress,
		uint8_t* scratchPad) {
...
void MKL_DallasTemperature::writeScratchPad(const uint8_t* deviceAddress,
...
// uses parasite mode.
// See issue #145
bool MKL_DallasTemperature::readPowerSupply(const uint8_t* deviceAddress)
{
...
void MKL_DallasTemperature::setResolution(uint8_t newResolution) {
...
bool MKL_DallasTemperature::setResolution(const uint8_t* deviceAddress,
...
uint8_t MKL_DallasTemperature::getResolution() {
	return bitResolution;
}

// returns the current resolution of the device, 9-12
// returns 0 if device not found
uint8_t MKL_DallasTemperature::getResolution(const uint8_t* deviceAddress) {
...
void MKL_DallasTemperature::setWaitForConversion(bool flag) {
	waitForConversion = flag;
}

// gets the value of the waitForConversion flag
bool MKL_DallasTemperature::getWaitForConversion() {
...
void MKL_DallasTemperature::setCheckForConversion(bool flag) {
	checkForConversion = flag;
}

// gets the value of the waitForConversion flag
bool MKL_DallasTemperature::getCheckForConversion() {
	return checkForConversion;
}

bool MKL_DallasTemperature::isConversionComplete() {
	uint8_t b = _wire->read_bit();
	return (b == 1);
}

// sends command for all devices on the bus to perform a temperature conversion
void MKL_DallasTemperature::requestTemperatures() {
...
bool MKL_DallasTemperature::requestTemperaturesByAddress(
...
void MKL_DallasTemperature::blockTillConversionComplete(uint8_t bitResolution) {
...
int16_t MKL_DallasTemperature::millisToWaitForConversion(uint8_t bitResolution) {
...
bool MKL_DallasTemperature::saveScratchPadByIndex(uint8_t deviceIndex) {
...
// Returns true if no errors were encountered, false indicates failure
bool MKL_DallasTemperature::saveScratchPad(const uint8_t* deviceAddress) {
...
// Returns true if no errors were encountered, false indicates failure
bool MKL_DallasTemperature::recallScratchPadByIndex(uint8_t deviceIndex) {
...
// If optional argument deviceAddress is omitted the command is send to all devices
// Returns true if no errors were encountered, false indicates failure
bool MKL_DallasTemperature::recallScratchPad(const uint8_t* deviceAddress) {
...
void MKL_DallasTemperature::setAutoSaveScratchPad(bool flag) {
  autoSaveScratchPad = flag;
}

// Gets the autoSaveScratchPad flag
bool MKL_DallasTemperature::getAutoSaveScratchPad() {
  return autoSaveScratchPad;
}

void MKL_DallasTemperature::activateExternalPullup() {
	if(useExternalPullup)
		digitalWrite(pullupPin, LOW);
}

void MKL_DallasTemperature::deactivateExternalPullup() {
	if(useExternalPullup)
		digitalWrite(pullupPin, HIGH);
}

// sends command for one device to perform a temp conversion by index
bool MKL_DallasTemperature::requestTemperaturesByIndex(uint8_t deviceIndex) {
...
float MKL_DallasTemperature::getTempCByIndex(uint8_t deviceIndex) {
...
// Fetch temperature for device index
float MKL_DallasTemperature::getTempFByIndex(uint8_t deviceIndex) {
...
int16_t MKL_DallasTemperature::calculateTemperature(const uint8_t* deviceAddress,
		uint8_t* scratchPad) {
...
int16_t MKL_DallasTemperature::getTemp(const uint8_t* deviceAddress) {
...
// DallasTemperature.h. It is a large negative number outside the
// operating range of the device
float MKL_DallasTemperature::getTempC(const uint8_t* deviceAddress) {
...
// DallasTemperature.h. It is a large negative number outside the
// operating range of the device
float MKL_DallasTemperature::getTempF(const uint8_t* deviceAddress) {
	return rawToFahrenheit(getTemp(deviceAddress));
}

// returns true if the bus requires parasite power
bool MKL_DallasTemperature::isParasitePowerMode(void) {
	return parasite;
}
...
void MKL_DallasTemperature::setUserData(const uint8_t* deviceAddress,
		int16_t data) {
...
int16_t MKL_DallasTemperature::getUserData(const uint8_t* deviceAddress) {
	int16_t data = 0;
	ScratchPad scratchPad;
	if (isConnected(deviceAddress, scratchPad)) {
...
int16_t MKL_DallasTemperature::getUserDataByIndex(uint8_t deviceIndex) {
	DeviceAddress deviceAddress;
	getAddress(deviceAddress, deviceIndex);
	return getUserData((uint8_t*) deviceAddress);
}

void MKL_DallasTemperature::setUserDataByIndex(uint8_t deviceIndex, int16_t data) {
	DeviceAddress deviceAddress;
	getAddress(deviceAddress, deviceIndex);
...
float MKL_DallasTemperature::toFahrenheit(float celsius) {
	return (celsius * 1.8f) + 32.0f;
}

// Convert float Fahrenheit to Celsius
float MKL_DallasTemperature::toCelsius(float fahrenheit) {
	return (fahrenheit - 32.0f) * 0.555555556f;
}

// convert from raw to Celsius
float MKL_DallasTemperature::rawToCelsius(int16_t raw) {
...
float MKL_DallasTemperature::rawToFahrenheit(int16_t raw) {
...
bool MKL_DallasTemperature::isAllZeros(const uint8_t * const scratchPad, const size_t length) {
...
void MKL_DallasTemperature::setHighAlarmTemp(const uint8_t* deviceAddress,
		int8_t celsius) {
...
// accepts a float, but the alarm resolution will ignore anything
// after a decimal point.  valid range is -55C - 125C
void MKL_DallasTemperature::setLowAlarmTemp(const uint8_t* deviceAddress,
		int8_t celsius) {
...
int8_t MKL_DallasTemperature::getHighAlarmTemp(const uint8_t* deviceAddress) {
...
// DEVICE_DISCONNECTED for an address
int8_t MKL_DallasTemperature::getLowAlarmTemp(const uint8_t* deviceAddress) {
...
void MKL_DallasTemperature::resetAlarmSearch() {
...
// MKL_DallasTemperature::resetAlarmSearch() to start over.
bool MKL_DallasTemperature::alarmSearch(uint8_t* newAddr) {
...
bool MKL_DallasTemperature::hasAlarm(const uint8_t* deviceAddress) {
...
bool MKL_DallasTemperature::hasAlarm(void) {
...
void MKL_DallasTemperature::processAlarms(void) {
...
void MKL_DallasTemperature::setAlarmHandler(const AlarmHandler *handler) {
	_AlarmHandler = handler;
}

// checks if AlarmHandler has been set.
bool MKL_DallasTemperature::hasAlarmHandler()
{
...
void* MKL_DallasTemperature::operator new(unsigned int size) { // Implicit NSS obj size
...
void MKL_DallasTemperature::operator delete(void* p) {
