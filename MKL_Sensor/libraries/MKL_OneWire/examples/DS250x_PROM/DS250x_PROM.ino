...
MKL_OneWire ds(6);                    // OneWire bus on digital pin 6
void setup() {
  Serial.begin (9600);
}
...
void loop() {
  ...
  if (present == true) {          // We only try to read the data if there's a device present
    ...
    ccrc_calc = MKL_OneWire::crc8(leemem, 3);  // We calculate the CRC of the commands we sent using the library function and store it
