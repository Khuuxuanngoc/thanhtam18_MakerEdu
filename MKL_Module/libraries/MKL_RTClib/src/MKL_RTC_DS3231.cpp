...
*/
/**************************************************************************/
bool MKL_RTC_DS3231::begin(TwoWire *wireInstance) {
  if (i2c_dev)
    delete i2c_dev;
...
*/
/**************************************************************************/
bool MKL_RTC_DS3231::lostPower(void) {
...
/*!
    ...
    @param dt MKL_DateTime object containing the date/time to set
*/
/**************************************************************************/
void MKL_RTC_DS3231::adjust(const MKL_DateTime &dt) {
...
/*!
    ...
    @return MKL_DateTime object with the current date/time
*/
/**************************************************************************/
MKL_DateTime MKL_RTC_DS3231::now() {
  uint8_t buffer[7];
  buffer[0] = 0;
  i2c_dev->write_then_read(buffer, 1, buffer, 7);

  return MKL_DateTime(bcd2bin(buffer[6]) + 2000U, bcd2bin(buffer[5] & 0x7F),
...
*/
/**************************************************************************/
Ds3231SqwPinMode MKL_RTC_DS3231::readSqwPinMode() {
  int mode;
...
*/
/**************************************************************************/
void MKL_RTC_DS3231::writeSqwPinMode(Ds3231SqwPinMode mode) {
...
*/
/**************************************************************************/
float MKL_RTC_DS3231::getTemperature() {
...
*/
/**************************************************************************/
bool MKL_RTC_DS3231::setAlarm2(const MKL_DateTime &dt, Ds3231Alarm2Mode alarm_mode) {
  uint8_t ctrl = read_register(DS3231_CONTROL);
  if (!(ctrl & 0x04)) {
    return false;
  }
...
/*!
    ...
    @return MKL_DateTime object with the Alarm1 data set in the
            day, hour, minutes, and seconds fields
*/
/**************************************************************************/
MKL_DateTime MKL_RTC_DS3231::getAlarm1() {
  ...
  return MKL_DateTime(2000, 5, day, hour, minutes, seconds);
}

/**************************************************************************/
/*!
    @brief  Get the date/time value of Alarm2
    @return MKL_DateTime object with the Alarm2 data set in the
            day, hour, and minutes fields
*/
/**************************************************************************/
MKL_DateTime MKL_RTC_DS3231::getAlarm2() {
  ...
  // On the first week of May 2000, the day-of-the-week number
  // matches the date number.
  return MKL_DateTime(2000, 5, day, hour, minutes, 0);
}
...
*/
/**************************************************************************/
Ds3231Alarm1Mode MKL_RTC_DS3231::getAlarm1Mode() {
  uint8_t buffer[5] = {DS3231_ALARM1, 0, 0, 0, 0};
  i2c_dev->write_then_read(buffer, 1, buffer, 5);
...
*/
/**************************************************************************/
Ds3231Alarm2Mode MKL_RTC_DS3231::getAlarm2Mode() {
  uint8_t buffer[4] = {DS3231_ALARM2, 0, 0, 0};
  i2c_dev->write_then_read(buffer, 1, buffer, 4);
...
*/
/**************************************************************************/
void MKL_RTC_DS3231::disableAlarm(uint8_t alarm_num) {
  uint8_t ctrl = read_register(DS3231_CONTROL);
  ctrl &= ~(1 << (alarm_num - 1));
...
*/
/**************************************************************************/
void MKL_RTC_DS3231::clearAlarm(uint8_t alarm_num) {
...
*/
/**************************************************************************/
bool MKL_RTC_DS3231::alarmFired(uint8_t alarm_num) {
...
*/
/**************************************************************************/
void MKL_RTC_DS3231::enable32K(void) {
  uint8_t status = read_register(DS3231_STATUSREG);
  status |= (0x1 << 0x03);
  write_register(DS3231_STATUSREG, status);
}
...
*/
/**************************************************************************/
void MKL_RTC_DS3231::disable32K(void) {
  uint8_t status = read_register(DS3231_STATUSREG);
  status &= ~(0x1 << 0x03);
  write_register(DS3231_STATUSREG, status);
}
...
*/
/**************************************************************************/
bool MKL_RTC_DS3231::isEnabled32K(void) {
